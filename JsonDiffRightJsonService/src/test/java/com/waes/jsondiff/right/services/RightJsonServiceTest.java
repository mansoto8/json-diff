...
/**
 * Units tests for the right persistence service
 */
@ExtendWith(MockitoExtension.class)
public class RightJsonServiceTest {
	...
	@Test
	public void persistRightJson_patch() throws URISyntaxException {
		...
		when(restTemplate.exchange(eq(callingURI), eq(HttpMethod.PATCH), any(HttpEntity.class),
				eq(Void.class))).thenReturn(expectedResponse);
		...
		verify(restTemplate).exchange(eq(callingURI), eq(HttpMethod.PATCH), any(HttpEntity.class),
				eq(Void.class));
	}

	@Test
	public void persistRightJson_post() throws URISyntaxException {
		URI mockURI = new URI("http://sample.uri");
		when(storeClient.getProfileUri()).thenReturn(mockURI);
		...
		when(restTemplate.getForEntity(eq("http://sample.uri/exists/1"), eq(Boolean.class))).thenReturn(mockResponse);
		ResponseEntity<Void> expectedResponse = new ResponseEntity<Void>(HttpStatus.OK);
		when(restTemplate.exchange(eq(mockURI), eq(HttpMethod.POST), any(HttpEntity.class),	
				eq(Void.class))).thenReturn(expectedResponse);
		...
		verify(storeClient).getProfileUri();
		verify(restTemplate).getForEntity(eq("http://sample.uri/exists/1"), eq(Boolean.class));
		verify(restTemplate).exchange(eq(mockURI), eq(HttpMethod.POST), any(HttpEntity.class),
				eq(Void.class));
	}
